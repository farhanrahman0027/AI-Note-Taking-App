...
export function NoteCard({ id, title, content, isPinned, tags, updatedAt, onDelete, onPin, onEdit }: NoteCardProps) {
  ...
  return (
    <Card className="p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => onEdit(id)}>
      <div className="space-y-2">
        <div className="flex items-start justify-between">
          <h3 className="font-semibold text-foreground truncate flex-1">{title || "Untitled"}</h3>
          <button
            onClick={(e) => {
              e.stopPropagation()
              onPin(id, !isPinned)
        ...
        <p className="text-sm text-muted-foreground line-clamp-2">{content || "No content"}</p>

        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {tags.map((tag) => (
              <span key={tag} className="inline-block px-2 py-1 text-xs bg-secondary text-secondary-foreground rounded">
                {tag}
              </span>
            ))}
          </div>
